...
configuration and TypeScript for a React widget — so the two can never drift
apart.
...
duplicate Zoho api_names, and bad rollup targets all fail the
build rather than producing a broken target.

## The model

13 entities: `households` `students` `teachers` `terms` `programs` `courses`
`holidays` `admissions` `classes` `class_sessions` `enrollments` `allocations` `attendance`.
